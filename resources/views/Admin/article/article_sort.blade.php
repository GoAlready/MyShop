       ...
       <thead>
        ...
        <?php foreach($cates as $v): ?>
            <tr>
                ...
                <td><?=$v->id?></td>
                <td><?=$v->catename?></td>
                <td class="displayPart" displayLength="60"><?=$v->descrip?></td>
                <td><?=$v->created_at?></td>
                <td>
                    @if($v->is_enable == 0)
                        禁用
                ...
                <td class="td-manage">   
                    <a title="编辑" onclick="member_edit('510')" href="{{route('admin_artsort_edit',['id' => $v['id']])}}"  class="btn btn-xs btn-info" ><i class="fa fa-edit bigger-120"></i></a>      
...
<div id="addsort_style" style="display:none" class="article_style">
    <form action="{{route('admin_artsort_add')}}" method="post">
        {{csrf_field()}}
        <div class="add_content" id="form-article-add">
            <ul>
                <li class="clearfix Mandatory"><label class="label_name"><i>*</i>分类名称:</label>
                <span class="formControls w_txt"><input name="catename" type="text" id="form-field-1" class="col-xs-7 col-sm-5 "></span>
                </li>
                
                <li class="clearfix"><label class="label_name">是否启用:</label>
                    <span class="formControls w_txt"><input name="is_enable" type="text" id="form-field-1" value="" class="col-xs-7 col-sm-2 "></span>
                </li>

                <li class="clearfix"><label class="label_name">简介:</label>
                <span class="formControls w_txt"><textarea name="descrip" class="form-control" id="form_textarea" placeholder="" onkeyup="checkLength(this);"></textarea>
                </li>

                <li class="clearfix">
                    <button class="formControls btn btn-primary" type="submit" value="" style="margin-left:45%;">提交</button>
                </li>
            </ul>
        </div>
    </form>
...
function member_del(obj,id){
	layer.confirm('确认要删除吗？',{icon:0,},function(index){
        ...
        $.ajax("/admin/artsort_delete?id="+id);
