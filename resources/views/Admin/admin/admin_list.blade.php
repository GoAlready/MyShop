...
<body>
	<div class="page-content clearfix">
		<div class="administrator">
			<div class="d_Confirm_Order_style">
				...
				<div class="border clearfix">
					...
					<span class="r_f">共：<b>{{$num}}</b>人</span>
				</div>
				<!--管理员列表-->
								...
								<div class="side_list">
									...
									<div class="widget-body">
										<ul class="b_P_Sort_list">
											<li><i class="fa fa-users green" value="0"></i> <a href="{{route('admin_adminlist',['id'=>'0'])}}">全部管理员({{$num}})</a></li>
											@foreach($role as $v)
											<li><i class="fa fa-users orange"></i> <a href="{{route('admin_adminlist',['id'=>$v['id']])}}">{{$v['role_name']}}</a></li>
											@endforeach
										</ul>
									</div>
								</div>
								...
								<?php foreach($admin as $v): ?>
									...
									<tr>
										...
										<td class="td-manage">
											...
											<a title="编辑" href="{{route('admin_adminedit',['id'=>$v->id])}}" class="btn btn-xs btn-info"><i
		...
		<div id="add_administrator_style" class="add_menber" style="display:none">
			<form action="{{route('admin_adminadd')}}" method="post" id="form-admin-add">
				@csrf
...
<script type="text/javascript">
	...
	function del()
	{
		...
		if(checked.length>=1)
		{
			...
			{
				$.ajax("/admin/admin_delete?id="+checked[i]);
			}
